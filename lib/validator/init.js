  ...
  light: function(params) {
    ...
    if (validatedParams['publisher'] === 'OST') {
      if (message['kind'] === 'event_received') {
        ...
        if (!util.valPresent(message['payload']['transactionUuid'])) {
          let errorParams = {
    ...
    // Specific events are validation for OST_CMD publisher.
    if (validatedParams['publisher'] === 'OST_CMD') {
      if (message['kind'] === 'holdWorker') {
        if (
          !util.valPresent(message['payload']['msg_id']) ||
          message['payload']['final_status'] ||
          !util.valPresent(message['payload']['client_id'])
        ) {
          let errorParams = {
            internal_error_identifier: 's_v_i_9',
            api_error_identifier: 'invalid_payload_for_holdWorker',
            error_config: errorConfig,
            debug_options: {}
          };
          return Promise.resolve(responseHelper.error(errorParams));
        }
      } else if (message['kind'] === 'redirectToDhq') {
        if (
          !util.valPresent(message['payload']['msg_id']) ||
          message['payload']['final_status'] ||
          !util.valPresent(message['payload']['client_id'])
          ...
          let errorParams = {
            internal_error_identifier: 's_v_i_10',
            api_error_identifier: 'invalid_payload_for_redirectToDhq',
            error_config: errorConfig,
            debug_options: {}
          };
          return Promise.resolve(responseHelper.error(errorParams));
        }
      } else if (message['kind'] === 'transactionsDone') {
        if (
          !util.valPresent(message['payload']['msg_id']) ||
          message['payload']['final_status'] ||
          !util.valPresent(message['payload']['client_id'])
          ...
          let errorParams = {
            internal_error_identifier: 's_v_i_11',
            api_error_identifier: 'invalid_payload_for_transactionsDone',
            error_config: errorConfig,
            debug_options: {}
          };
          return Promise.resolve(responseHelper.error(errorParams));
        }
      } else if (['startDhq', 'redirectToDq', 'startDq'].includes(message['kind'])) {
        if (!util.valPresent(message['payload']['msg_id'])) {
          let errorParams = {
            internal_error_identifier: 's_v_i_12',
            api_error_identifier: 'invalid_payload_for_command_message',
            error_config: errorConfig,
            debug_options: {}
          };
          return Promise.resolve(responseHelper.error(errorParams));
        ...
        let errorParams = {
          internal_error_identifier: 's_v_i_13',
          api_error_identifier: 'unsupported_command_message_kind',
          error_config: errorConfig,
          debug_options: {}
        };

        logger.error('Unsupported kind (' + message['kind'] + ') transferred.');
        return Promise.resolve(responseHelper.error(errorParams));
      }
