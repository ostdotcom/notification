    ...
    if (validatedParams['publisher'] == 'OST') {
      ...
      if (message['kind'] == 'event_received') {
        ...
        if (
          ...
          return Promise.resolve(responseHelper.error('s_v_i_5', 'Invalid payload for kind email'));
        }
        ...
        if (
          ...
          return Promise.resolve(responseHelper.error('s_v_i_6', 'Invalid payload for kind shared_entity'));
        }
      } else if(message['kind'] == 'execute_transaction') {
        if (
          !util.valPresent(message['payload']['transactionLogId']) ||
          !util.valPresent(message['payload']['transactionUuid'])
        ) {
          return Promise.resolve(responseHelper.error('s_v_i_7', 'Invalid payload for kind execute_transaction'));
        }
        ...
        logger.error('s_v_i_8', 'Unsupported kind ('+message['kind']+') transfered. supported are event_received,transaction_initiated,transaction_mined');
